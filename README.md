...
> Placeholder — kendi ekran görüntülerinizi `docs/screenshots/` altına ekleyin.

| Görünüm | Açıklama |
|---------|----------|
| `docs/screenshots/twin-dashboard.png` | Ana dashboard + telemetri paneli |
| `docs/screenshots/twin-3d-night.png` | Gece modu + emissive binalar |
| `docs/screenshots/twin-flight-path.png` | Drone uçuş yolu + ısı haritası |
| `docs/screenshots/twin-analytics.png` | Analitik tablo + sparkline |
...
| Özellik | Açıklama |
|---------|----------|
| **Post-processing** | FXAA + bloom (sol panelden aç/kapa) |
| **Gelişmiş su** | Animasyonlu dalga shader, fresnel, gökyüzü yansıması |
| **Arazi ısı haritası** | scene_data.json yeşil/beton/toprak oranları |
| **Drone uçuş yolu** | mosaic_meta stitch sırası + animasyonlu marker |
| **Gece pencereleri** | Gün saati düşükken binalarda emissive ışık |
| **Bina arama** | ID, tip, durum filtreleme |
| **Klavye kısayolları** | R/W/D/M/H — yardım overlay |
| **Onboarding turu** | İlk ziyarette rehber (sessionStorage) |
| **Daraltılabilir paneller** | localStorage ile durum hatırlama |
| **Açık/koyu tema** | Tema toggle |
| **Toast alarmları** | Yeni alarm bildirimleri |
| **Sparkline grafikler** | Son 60 dk sıcaklık/rüzgar trendi |
| **Varlık tablosu** | 61 bina, sıralanabilir, durum rozetleri |
| **Saha istatistikleri** | Toplam footprint m², ort. bina yüksekliği |
| **Rapor export** | JSON indir (site meta + seçili bina) |
| **Paylaşım linki** | Kamera preset + dataset query params |
| **WebSocket telemetri** | `/ws/telemetry` (fallback: polling) |
| **Telemetri geçmişi** | `GET /api/telemetry/history?minutes=60` |
| **CI** | GitHub Actions — import check + ruff |
| **run_twin.py --open** | Tarayıcıyı otomatik aç |

### API uçları (`twin_api.py` / `serve.py`)

```
GET /api/telemetry          — ortam + bina telemetrisi
GET /api/telemetry/history  — son N dakika geçmişi (?minutes=60)
GET /api/assets             — varlık kaydı (buildings.json genişletilmiş)
GET /api/alarms             — eşik tabanlı alarmlar
WS  /ws/telemetry           — gerçek zamanlı push (serve.py)
```
